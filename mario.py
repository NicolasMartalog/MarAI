...
# using GPU for computation (RTX 2060)
device = torch.device("cuda")


# Preprocess Environment (From Pytorch documentation)
...
class PPOSolver:
    ...
    def __init__(self):
        ...
        self.n_mini_batch = 4
        self.epochs = 30
        self.save_directory = "H:/Projects/MarioML/checkpointsw8l4"  # set custom save directory path
        self.batch_size = self.worker_steps
        self.mini_batch_size = self.batch_size // self.n_mini_batch
        self.obs = env.reset().__array__()
        self.policy = MarioModel().to(device)
        self.mse_loss = nn.MSELoss()
        self.optimizer = torch.optim.Adam([
            {'params': self.policy.actor.parameters(), 'lr': 0.00025},
            {'params': self.policy.critic.parameters(), 'lr': 0.001}
        ], eps=1e-4)
        self.policy_old = MarioModel().to(device)
        self.policy_old.load_state_dict(self.policy.state_dict())
        self.all_episode_rewards = []
        self.all_mean_rewards = []
        self.episode = 0  # set custom episode (ex, for starting at 4000 episode start at 4000)

    # Saves last checkpoint to save_directory path
    ...
    # Loads last checkpoint from save_directory path
    ...
    # Sets initial state and goes through training episodes for average rewards/saves rewards
    def sample(self):
    ...
    # This calculates the general advantage estimation
    ...
    # MSE loss from the actual results and the estimations
...
solver = PPOSolver()
#solver.load_checkpoint('checkpoint_4000.pth') can load from custom checkpoint
# While loop for training the Mario model until user stops the program
while True:
